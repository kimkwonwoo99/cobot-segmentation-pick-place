...
def gripper_callback(data):
    ...
    mc.set_gripper_value(data,50)
    
    while True:
        gripper_value = mc.get_gripper_value()
        if gripper_value == data:
            break
    rospy.loginfo("gripper value is %d", data)
    return True
...
def listener():
    ...
    rospy.Subscriber("move_start_topic", Int32 , state_callback)
    rospy.Service("calibrate_service", serviceMessagetype , cali_callback)
    rospy.Service("gripper_service", serviceMessagetype , gripper_callback)
    time.sleep(0.02)
    mc.set_fresh_mode(1)
    time.sleep(0.03)
    print("spin ...")
