...
class MoveToGoal:
    ...
    #         success = self.cancel_pub.publish(self.cancel_msg)
    #         if success:
    #             rospy.loginfo("Successfully Stopped")
    #         else:
    #             rospy.logwarn("Failed to stop")
    #     else:
    ...
    def user_request_callback(self, msg):
        ...
        if msg.data == 'stop':
            self.cancel_goal()
            # success = self.ac.cancel_all_goals()
            # success = self.cancel_pub.publish(self.cancel_msg)
        if msg.data not in ['stop', 'bathroom', 'livingroom', 'dressroom']:
                rospy.logwarn("Invalid input")
